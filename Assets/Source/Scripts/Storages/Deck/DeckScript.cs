...
namespace Xdd.Scripts.Hands
{
    ...
    internal class DeckScript : MonoBehaviour
    {
        ...
        public ITaskItem AddCard(TaskQueue manager, CardMono[] cards)
        {
            ...
            var task = manager.AddTask((x) => MoveCards(cards, positions), name: $"move to deck {cards.Length} cards");
            task.OnEnd += (x) =>
            {
        ...
        private async Task MoveCards(CardMono[] cards, Vector3[] positions)
        {
